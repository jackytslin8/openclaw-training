[電費趨勢_echarts.html](https://github.com/user-attachments/files/28584413/_echarts.html)# 🤖 OpenClaw AI Agent
...
- **效果**：完全自動，還能計算信用卡回饋
[Uploading 電費<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>自治路557號 電費趨勢</title>
  <script src="https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js"></script>
  <style>
    body { font-family: 'Segoe UI', sans-serif; background: #1a1a2e; margin: 0; padding: 20px; }
    #chart { width: 100%; height: 500px; }
    .info { color: #aaa; max-width: 900px; margin: 0 auto 20px; font-size: 13px; line-height: 1.6; }
...
<div class="info">
  自治路557號｜電費趨勢（2024/11 – 2026/03）｜電號：<strong>21140184105</strong>（1樓）、<strong>21140184116</strong>（2-3樓）
</div>
<div id="chart"></div>
...
const floor1  = [3320, 1675, 1519, 1813, 3248, 4297, 3531, 1712, 1517];
const floor23 = [3436, 1093,  924, 1429, 4232, 7008, 4475, 1136, 1253];
const total   = floor1.map((v, i) => v + floor23[i]);

const option = {
  backgroundColor: '#1a1a2e',
  textStyle: { color: '#e0e0e0', fontFamily: 'Segoe UI' },
  tooltip: {
    trigger: 'axis',
    axisPointer: { type: 'shadow' },
    formatter: function(params) {
      let s = params[0].name.replace('\n', ' ') + '<br/>';
      params.forEach(function(p) {
        if (p.seriesName !== '合計') {
          s += p.marker + ' ' + p.seriesName + '：<strong>' + p.value.toLocaleString() + ' 元</strong><br/>';
        }
      });
      var last = params[params.length - 1];
      s += '<span style="display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background:#ffd700"></span> ';
      s += '合計：<strong>' + last.value.toLocaleString() + ' 元</strong>';
      return s;
    }
  },
  legend: {
  ...
  yAxis: {
    ...
    axisLabel: { color: '#aaa', formatter: function(v) { return v.toLocaleString(); } },
    axisLine: { lineStyle: { color: '#444' } },
  ...
  series: [
    ...
    },
    {
      ...
      itemStyle: { color: '#ffd700', borderWidth: 2, borderColor: '#1a1a2e' },
      label: {
        show: true,
        position: 'top',
        color: '#ffd700',
        fontWeight: 'bold',
        formatter: function(v) { return v.value.toLocaleString(); }
      },
      smooth: 0.3,
      markLine: {
        silent: true,
        lineStyle: { color: '#ff5252', type: 'dashed', width: 1.5 },
        label: { show: true, text: '夏月', color: '#ff5252', fontSize: 11 },
        data: [{ xAxis: 5 }]
      },
      markArea: {
        silent: true,
